...
import { Palette, Accessibility, Wrench, Brain, Pencil, HelpCircle, Sun, Moon, Globe } from "lucide-react";
...
export function Navbar() {
  ...
  const handleModeToggle = (dark: boolean) => {
    setIsDark(dark);
    localStorage.setItem("mm_dark_mode", String(dark));
    if (dark) {
              ...
              <div className="px-5 pb-4 pt-2 border-t border-slate-200 dark:border-slate-700">
                ...
                <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-3">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-medium transition-all touch-manipulation ${
                      isDark
